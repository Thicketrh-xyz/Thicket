...
export function Verification() {
  return (
    <>
      ...
      <p>Paid jobs can run this way too, and when one does the buyer receives the answer the majority agreed on rather than whatever the fastest node returned. Verifying everything would triple the network's compute, so this is sampled rather than universal — most work is policed by the risk of being checked rather than by being checked.</p>
      <p>How much is sampled is a live setting, not a fixed property: read <code>spot_check_rate</code> from <code>{`GET /stats`}</code>. <strong>At zero, no paid job is cross-checked at all</strong> and every buyer gets a single node's unverified answer. Challenges are unaffected by that setting and are always verified.</p>
